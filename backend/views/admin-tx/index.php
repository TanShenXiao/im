...
use yii\helpers\Url;
$modelLabel = new \backend\models\MemberUser();
?>
            ...
            <div class="box">
                ...
                <div class="box-header">
                    ...
                    <div class="box-tools">
                        <div class="input-group input-group-sm" style="width: 150px;">
                            <button id="delete_btn" type="button" class="btn btn-xs btn-primary">批量操作</button>
                        </div>
                    </div>
                </div>
                <!-- /.box-header -->
                ...
                <div class="box-body">
                    <div id="example1_wrapper" class="dataTables_wrapper form-inline dt-bootstrap">
                        ...
                        <div class="row">
                            <div class="col-sm-12">
                                <?php ActiveForm::begin(['id' => 'admin-module-search-form', 'method'=>'get', 'options' => ['class' => 'form-inline'], 'action'=>Url::toRoute('admin-tx/index')]); ?>
                                ...
                                <div class="form-group" style="margin: 5px;">
                                    <label><?=$modelLabel->getAttributeLabel('nickname')?>:</label>
                                    <input type="text" class="form-control" id="query[nickname]" name="query[name]"  value="<?=isset($query["nickname"]) ? $query["nickname"] : "" ?>">
                                </div>

                                <div class="form-group" style="margin: 5px;">
                                    <label><?=$modelLabel->getAttributeLabel('alias')?>:</label>
                                    <input type="text" class="form-control" id="query[alias]" name="query[alias]"  value="<?=isset($query["alias"]) ? $query["alias"] : "" ?>">
                                </div>
                            ...
                            <div class="col-sm-12">
                                <table id="data_table" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="data_table_info">
                                    ...
                                    <tr role="row">
                                        ...
                                        echo '<th tabindex="0" aria-controls="data_table" rowspan="1" colspan="1" aria-sort="ascending" >'.$modelLabel->getAttributeLabel('nickname').'</th>';
                                        echo '<th tabindex="0" aria-controls="data_table" rowspan="1" colspan="1" aria-sort="ascending" >'.$modelLabel->getAttributeLabel('alias').'</th>';
                                        echo '<th tabindex="0" aria-controls="data_table" rowspan="1" colspan="1" aria-sort="ascending" >'.$modelLabel->getAttributeLabel('is_chat').'</th>';
                                        ?>
                                    ...
                                    foreach ($models as $model) {
                                        ...
                                        echo '  <td>' . $model->alias . '</td>';
                                        if($model->is_chat){
                                            echo '<td><a  class="btn btn-primary btn-sm" href="#">有权限</a></td>';
                                        }else{
                                            echo '<td><a  class="btn btn-danger btn-sm" href="#">禁言中</a></td>';
                                        }
                                        if($model->is_chat){
                                            echo '<td>
                                                    <a id="delete_btn" onclick="deleteAction(' . $model->id . ')" class="btn btn-danger btn-sm" href="#">禁言</a>
                                                    <a id="delete_btn" onclick=editAction(' . $model->id . ','.'"'.$model->alias.'"'.'); class="btn btn-primary btn-sm" href="#">修改用户聊天名</a>
                                                  </td>';

                                        }else{
                                            echo '<td>
                                                    <a id="delete_btn" onclick="deleteAction(' . $model->id . ')" class="btn btn-primary btn-sm" href="#">解禁</a>
                                                    <a id="delete_btn" onclick="editAction(' . $model->id . ')" class="btn btn-primary btn-sm" href="#">修改用户聊天名</a>
                                                  </td>';
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form "id"="admin-module-form" class="form-horizontal">
                <input type="hidden" class="form-control" id="id" name="AdminModule[id]" />


              <div id="code_div" class="form-group">
                    <label for="code" class="col-sm-2 control-label"><?php echo $modelLabel->getAttributeLabel("alias")?></label>
                    <div class="col-sm-10">
                        <input type="hidden" id="aliasid" value="" >
                        <input type="text" class="form-control" id="alias" value="" placeholder="必填" />
                    </div>
...
<script>
    ...
    function deleteAction(id){
        ...
        if(ids.length > 0){
            admin_tool.confirm('是否改变当前的发言状态', function(){
                $.ajax({
                    type: "GET",
                    url: "<?=Url::toRoute('admin-tx/delete')?>",
                    data: {"ids":ids},
                    cache: false,
                    dataType:"json",
                    error: function (xmlHttpRequest, textStatus, errorThrown) {
    ...
    $('#edit_dialog_ok').click(function (e) {
        ...
        $.ajax({
            type: "GET",
            url: "<?=Url::toRoute('admin-tx/realias')?>",
